...
public class NetherItems extends ItemRegistry {
    ...
    public static Item makeEgg(
        ...
        if (Configs.MOBS.getBoolean("mobs", name, true)) {
            Item egg = new ForgeSpawnEggItem(type, background, dots, defaultSettings());
            return NetherItems.registerItem(name, egg);
        } else {
            return Items.AIR;
        }
    }
    ...
    public static Item makeEgg(String name, EntityType<? extends Mob> type, int background, int dots) {
        return makeEgg(name, () -> type, background, dots);
    }
